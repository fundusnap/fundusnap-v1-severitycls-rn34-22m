...
base_model: timm/resnet34.tv_in1k
...
metrics:
  ...
  - recall
model-index:
  - name: fundusnap-v1-severitycls-rn34-22m
    results:
      - task:
          type: image-classification
          name: Diabetic Retinopathy Severity Grading
        dataset:
          type: resized-2015-2019-diabetic-retinopathy-detection
          name: Resized 2015 & 2019 Diabetic Retinopathy Detection (EyePACS + APTOS, Kaggle)
          split: validation
        metrics:
          - type: accuracy
            name: Accuracy
          ...
          - type: recall
            ...
            args:
              average: macro
extra_gated_prompt: >-
  This model analyses medical images and is released for research and engineering use only. It is
  not a medical device, has no regulatory clearance, and has not been prospectively validated.
  Please confirm the following before downloading the weights.
extra_gated_fields:
  I understand this model is not a medical device and has no regulatory clearance: checkbox
  I will not use it as the sole basis for any diagnosis, referral, or treatment decision: checkbox
  I will use it for non-commercial purposes only, consistent with the CC BY-NC licence: checkbox
